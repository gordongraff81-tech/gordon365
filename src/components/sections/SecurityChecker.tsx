...
function SeverityIcon({ severity }: { severity: "critical" | "warning" | "ok" }) {
  if (severity === "critical") {
    return (
      <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: "rgba(248,113,113,0.10)", border: "1px solid rgba(248,113,113,0.25)" }}>
  ...
  if (severity === "warning") {
    return (
      <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: "rgba(251,191,36,0.10)", border: "1px solid rgba(251,191,36,0.25)" }}>
  ...
  return (
    <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: "rgba(34,211,238,0.10)", border: "1px solid rgba(34,211,238,0.25)" }}>
...
function AnswerOption({
  ...
  const accentMap: Record<AnswerValue, { border: string; bg: string; dot: string; glow: string }> = {
    yes:     { border: "rgba(34,211,238,0.40)",  bg: "rgba(34,211,238,0.06)",  dot: "#22d3ee", glow: "rgba(34,211,238,0.3)"  },
    partial: { border: "rgba(251,191,36,0.40)",  bg: "rgba(251,191,36,0.06)",  dot: "#fbbf24", glow: "rgba(251,191,36,0.3)"  },
    no:      { border: "rgba(248,113,113,0.40)", bg: "rgba(248,113,113,0.06)", dot: "#f87171", glow: "rgba(248,113,113,0.3)" },
    unknown: { border: "rgba(255,255,255,0.10)", bg: "rgba(8,8,18,0.6)",       dot: "#64748b", glow: "transparent"            },
  };
  const ca = accentMap[value];
  ...
  return (
    <button
      ...
      style={{
        background: selected ? ca.bg : "rgba(8,8,18,0.5)",
        border: `1px solid ${selected ? ca.border : "rgba(255,255,255,0.07)"}`,
        boxShadow: selected ? `0 0 16px ${ca.glow}, inset 0 1px 0 rgba(255,255,255,0.05)` : "none",
      }}
    >
      <div
        className="w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 mt-0.5 transition-all"
        style={{
          borderColor: selected ? ca.dot : "rgba(255,255,255,0.15)",
          background: selected ? ca.dot : "transparent",
        }}
      >
        {selected && <div className="w-1.5 h-1.5 rounded-full bg-white" />}
      </div>
      <div>
        <div className="text-[0.875rem] font-semibold leading-snug" style={{ color: "#f1f5f9" }}>{title}</div>
        <div className="text-[0.75rem] mt-0.5 leading-snug" style={{ color: "rgba(100,116,139,0.7)" }}>{hint}</div>
      </div>
    </button>
  );
}
...
function ProgressBar({ current, total }: { current: number; total: number }) {
  return (
    <div className="flex items-center gap-3 mb-6">
      ...
      <span className="text-[0.6875rem] font-bold tracking-[0.08em] uppercase whitespace-nowrap flex-shrink-0" style={{ color: "rgba(100,116,139,0.7)" }}>
        {current} / {total}
      </span>
    </div>
  );
...
function ResultsScreen({
  ...
  const postureColors = {
    strong:    { color: "#22d3ee", bg: "rgba(34,211,238,0.08)",    border: "rgba(34,211,238,0.25)"  },
    attention: { color: "#fbbf24", bg: "rgba(251,191,36,0.08)",    border: "rgba(251,191,36,0.25)"  },
    critical:  { color: "#f87171", bg: "rgba(248,113,113,0.08)",   border: "rgba(248,113,113,0.25)" },
  };
  const pc = postureColors[posture];
  ...
  return (
    ...
    >
      {/* Score Header */}
      <div className="flex items-center gap-6 mb-6 pb-6" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
        ...
        <div>
          <div
            ...
            style={{ color: pc.color, background: pc.bg, borderColor: pc.border, textShadow: `0 0 8px ${pc.color}60` }}
          >
            <span className="w-1.5 h-1.5 rounded-full bg-current" />
            {tc(`results.postures.${posture}`)}
          </div>
          <h3 className="font-display font-bold text-[1.0625rem] leading-snug mb-1" style={{ color: "#f1f5f9" }}>
            {tc(`results.headlines.${posture}`)}
          </h3>
          <p className="text-[0.8125rem] leading-relaxed max-w-[320px]" style={{ color: "rgba(148,163,184,0.72)" }}>
      ...
      <div className="flex items-center gap-2 mb-5">
        {criticalCount > 0 && (
          <span className="text-[0.6875rem] font-bold px-2.5 py-1 rounded-full" style={{ background: "rgba(248,113,113,0.10)", border: "1px solid rgba(248,113,113,0.20)", color: "#f87171" }}>
            {criticalCount} Kritisch
          </span>
        )}
        {warningCount > 0 && (
          <span className="text-[0.6875rem] font-bold px-2.5 py-1 rounded-full" style={{ background: "rgba(251,191,36,0.10)", border: "1px solid rgba(251,191,36,0.20)", color: "#fbbf24" }}>
            {warningCount} Warnung
          </span>
        )}
        {okCount > 0 && (
          <span className="text-[0.6875rem] font-bold px-2.5 py-1 rounded-full" style={{ background: "rgba(34,211,238,0.10)", border: "1px solid rgba(34,211,238,0.20)", color: "#22d3ee" }}>
            {okCount} OK
          </span>
        )}
      </div>
      ...
      {/* Findings List */}
      <h4 className="text-[0.6875rem] font-bold tracking-[0.08em] uppercase mb-3" style={{ color: "rgba(100,116,139,0.7)" }}>
        ...
        {[...findings]
          ...
          .map((f, i) => (
            ...
            >
              ...
              <div>
                <div className="text-[0.875rem] font-semibold mb-0.5 leading-snug" style={{ color: "#f1f5f9" }}>{f.title}</div>
                <div className="text-[0.8125rem] leading-relaxed" style={{ color: "rgba(148,163,184,0.7)" }}>{f.desc}</div>
              </div>
            </div>
          ))}
      </div>
        ...
        >
          ...
          <div className="hidden lg:block">
            ...
            >
              ...
              <span className="text-[0.8125rem] font-bold" style={{ color: "#6366f1" }}>{tc("title")}</span>
              <span
            ...
            <p className="text-[0.875rem] mb-8" style={{ color: "rgba(100,116,139,0.7)" }}>{tc("desc")}</p>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      opacity: answered || active ? 1 : 0.5,
                    }}
                  >
                    <div
                      className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 text-[0.6875rem] font-bold transition-all"
                      style={
                        answered
                          ? { background: "rgba(34,211,238,0.14)", border: "1px solid rgba(34,211,238,0.30)", color: "#22d3ee" }
                          : active
                          ? { background: "linear-gradient(135deg, #6366f1, #22d3ee)", color: "#fff", boxShadow: "0 0 10px rgba(99,102,241,0.4)" }
                    ...
                    <span
                      className="text-[0.8125rem] leading-snug line-clamp-1"
                      style={{ color: active ? "#f1f5f9" : "rgba(148,163,184,0.65)", fontWeight: active ? 600 : 400 }}
                    >
                      {q.text}
                    </span>
                  </div>
          ...
          <div
            ...
            style={{
              ...
              boxShadow: "0 0 0 1px rgba(99,102,241,0.08), 0 0 48px rgba(99,102,241,0.10), 0 24px 48px rgba(0,0,0,0.55), inset 0 1px 0 rgba(255,255,255,0.05)",
                ...
                <>
                  ...
                  <AnimatePresence mode="wait" custom={direction}>
                    ...
                    >
                      {/* Question */}
                      <div className="mb-5">
                        <div className="text-[0.6875rem] font-bold tracking-[0.08em] uppercase mb-2" style={{ color: "rgba(100,116,139,0.7)" }}>
                          {tc("question", { current: currentQ, total: TOTAL_QUESTIONS })}
                        </div>
                        <h3 className="font-display font-bold text-[1.0625rem] leading-snug mb-2" style={{ color: "#f1f5f9" }}>
                          {question.text}
                        </h3>
                      ...
                      style={
                        canProceed
                          ? {
                              ...
                              color: "#fff",
                              boxShadow: "0 0 24px rgba(99,102,241,0.4), 0 0 48px rgba(34,211,238,0.15)",
                            }
                          : {
